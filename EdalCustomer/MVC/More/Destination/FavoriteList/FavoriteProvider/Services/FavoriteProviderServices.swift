...
class FavoriteProviderServices{
    ...
    func getFavoriteProvider(page: Int, completion: @escaping (_ error: String?, _ data: FavoriteListModel?) -> Void){
        ApiClient.CallApi(endPoint: .favorite(page: page)) { (data: FavoriteListModel?, error: Error?, code) in
